...
a.imshow(np.reshape(train_face[0,:],[112,92]))#显示图片
plt.xlabel('人脸')
a=fig.add_subplot(1,2,2)
p=train_face.T*v
a.imshow(np.reshape(p[:,0],[112,92]))#显示图片
plt.xlabel('pca特征脸')
...
for i in range(num_test):
    ...
    #sqDistances=sqDistances/sqDistances.max()
...
#==============================================================================
label1=int(file_path1.split('/')[-2][1:]) # 目标脸所属类别。注：这个是因为图片所属文件夹的数字就是他对应的类别，比如s1，就是第一类，所以提取出1
...
temp_face = test_face - data_mean# 对待测图片进行pca
data_test_new = temp_face * V_r  # 得到测试脸在特征向量下的数据
